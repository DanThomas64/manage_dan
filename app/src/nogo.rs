...
impl SystemsStatus {
    ...
    pub fn init(&mut self) -> SystemsStatus {
        ...
        // 2. Initialize log system — must happen after DB so 'log' table exists.
        ...
        // initialize todo (Vikunja or nb backend, per config)
        let cfg = AppConfig::get();
        match todo::init(
            &cfg.todo.backend,
            &cfg.vikunja.base_url,
            &cfg.vikunja.api_token,
            cfg.vikunja.project_id,
            &cfg.todo.nb_notebook,
        )
